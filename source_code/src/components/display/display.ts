...
export default defineComponent({
  ...
  setup(props) {
    ...
    function getColorOfChar(charObject: CharObject) {
      if (
        settings.value?.saturate === true &&
        settings.value?.luminance === true
      ) {
      ...
      if (settings.value?.saturate === true) {
        ...
        return `rgb(${charObject.r + maxDiff}, ${charObject.g + maxDiff}, ${
          charObject.b + maxDiff
        })`;
      }
    ...
    function buildAscii() {
      ...
      for (let j = 0; j < picture.value.height; j++) {
        ...
        for (let i = 0; i < picture.value.width; i++) {
          ...
          const charIndex = p5.value.floor(
            p5.value.map(
              avg,
              0,
              255,
              settings.value?.invert ? 0 : len,
              settings.value?.invert ? len : 0
            )
          );
    ...
    function loadAsciiIntoHTML() {
      ...
      ascii.value = asciiObjects
        .map((rowObject: CharObject[]) =>
          rowObject.map((charObject: CharObject) => charObject.c).join("")
        )
        .join("\n");
      const html = asciiObjects
        .map((rowObject: CharObject[]) => {
          const mappedLine = rowObject
            .map((charObject: CharObject) => {
              return `
            <p
              style="
    ...
    async function handleUpdateFile(
      ...
      if (props.file.value) {
        ...
        await waitTickAmount(
          Math.min(Math.ceil(props.file.value.size / 1000), 50)
        );
        ...
        if (overwriteSettingsSize) {
          settings.value = settings.value
            ? {
                ...
                width: 50,
                height: Math.round(
                  picture.value.height / (picture.value.width / 50)
                ),
              }
            : undefined;
        }
    ...
    function handleSettingsSync(newSettings: ISettings | undefined) {
      ...
      if (
        settings.value?.width !== newSettings?.width ||
        settings.value?.height !== newSettings?.height
      ) {
        handleUpdateFile();
      }
    ...
    async function handleDownloadClick(format: string) {
      loading.value = true;
      ...
      await downloadImageFromHtmlElement(canvas.value, format);
      if (hs || vs) {
